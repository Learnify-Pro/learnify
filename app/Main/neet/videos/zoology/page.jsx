...
const ZoologyVideos = () => {
  ...
  const fetchFiles = async () => {
    ...
    const zoologyFiles = filesList.filter(file => file.subject === 'ZOOLOGY');
    setFiles(zoologyFiles);
    setLoading(false);
  };
  ...
  const filteredFiles = files.filter(file => {
    ...
    const nameMatch = file.videoName.toLowerCase().includes(searchQuery); // New filter condition by video name
    const isNEET = file.course.trim().toLowerCase() === 'neet';
    return institutionMatch && subjectMatch && (searchMatch || nameMatch) && isNEET;
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-center mt-4">
        ...
        <h1 className="ml-2 font-semibold text-md absolute mt-4 left-16 hidden md:flex">Zoology Videos</h1>
        <span className="mr-2 ml-2 border text-xl mt-4 rounded-md p-2">
          <IoSearch />
        </span>
        <Input
          placeholder='Search...'
