  ...
  const filteredFiles = files.filter(file => {
    ...
    const searchMatch = searchQuery === '' ||
      (isYearQuery ? (file.fileYear && file.fileYear.toString() === searchQuery) :
        (file.fileName && file.fileName.toLowerCase().includes(searchQuery)) || tags.includes(searchQuery) || (file.course && file.course.toLowerCase().includes(searchQuery)));

    // Check if the tags contain "NEET" or any of the specified subjects
    const containsRelevantTag = tags.includes("neet") || 
      (file.fileName && file.fileName.toLowerCase().includes("neet")) || 
      (file.course && file.course.toLowerCase().includes("neet")) ||
      tags.includes("biology") ||
      tags.includes("zoology") ||
      tags.includes("botany") ||
      tags.includes("chemistry") ||
      tags.includes("physics") ||
      (file.fileName && file.fileName.toLowerCase().includes("biology")) ||
      (file.fileName && file.fileName.toLowerCase().includes("zoology")) ||
      (file.fileName && file.fileName.toLowerCase().includes("botany")) ||
      (file.fileName && file.fileName.toLowerCase().includes("chemistry")) ||
      (file.fileName && file.fileName.toLowerCase().includes("physics")) ||
      (file.course && file.course.toLowerCase().includes("biology")) ||
      (file.course && file.course.toLowerCase().includes("zoology")) ||
      (file.course && file.course.toLowerCase().includes("botany")) ||
      (file.course && file.course.toLowerCase().includes("chemistry")) ||
      (file.course && file.course.toLowerCase().includes("physics"));

    const isRelevantSubjectSelected = selectedSubject === 'All Subjects' || selectedSubject.toLowerCase() === 'neet' ||
      selectedSubject.toLowerCase() === 'biology' || selectedSubject.toLowerCase() === 'zoology' ||
      selectedSubject.toLowerCase() === 'botany' || selectedSubject.toLowerCase() === 'chemistry' ||
      selectedSubject.toLowerCase() === 'physics';

    return institutionMatch && subjectMatch && searchMatch && containsRelevantTag && isRelevantSubjectSelected;
  });
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center justify-center mt-4">
        <button onClick={handleBack} className="mr-2 hidden md:flex ml-2 absolute left-8 border text-xl rounded-md p-2">
          <FaArrowLeft />
        </button>
          ...
          filteredFiles.length > 0 ? (
            ...
            >
              <div className="text-center w-full flex flex-col items-center content mt-8 justify-center">
                ...
                <p className="text-gray-500 mt-8">No PDFs found.</p>
              </div>
            </motion.div>
          )
        )}
